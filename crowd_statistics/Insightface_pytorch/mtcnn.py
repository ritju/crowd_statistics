...
class MTCNN():
    ...
    def align(self, img):
        ...
        warped_face = warp_and_crop_face(img, facial5points, self.refrence, crop_size=(112,112))

        # 将图片拷贝到tensor
        img_w,img_h = warped_face.size()
        gpuTensor = torch.zeros((img_h,img_w,3),device=self.device,dtype=torch.uint8)
        cpy = cuda.CUDA_MEMCPY2D()
        cpy.WidthInBytes = img_w*3
        cpy.Height = img_h
        cpy.srcMemoryType = cuda.CUmemorytype.CU_MEMORYTYPE_DEVICE
        cpy.srcDevice = warped_face.cudaPtr()
        cpy.srcXInBytes = 0
        cpy.srcY = 0
        cpy.srcPitch = warped_face.step
