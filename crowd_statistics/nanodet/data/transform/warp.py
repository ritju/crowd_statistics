...
class ShapeTransform:
    ...
    def __call__(self, meta_data, dst_shape):
        ...
        M = ResizeM @ M
        gpuMat_cv = cv2.cuda.createGpuMatFromCudaMemory(raw_img.shape[0],raw_img.shape[1],cv2.CV_8UC3,raw_img.data_ptr())

        img = cv2.cuda.warpPerspective(gpuMat_cv, M, dsize=tuple(dst_shape))
        # img1 = cv2.warpPerspective(raw_img.cpu().detach().numpy(), M, dsize=tuple(dst_shape))
        # img2 = cv2.warpPerspective(gpuMat_cv.download(), M, dsize=tuple(dst_shape))

        # print((raw_img.cpu().detach().numpy() == gpuMat_cv.download()).all())
        # print((img1==img2).all())
        # print((img == img1).all())
